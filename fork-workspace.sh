...
# fork-workspace.sh — Create a customized fork of this auto-tune workspace.
#
# Usage:
...
if [ -z "$TARGET_DIR" ]; then
    ...
    echo ""
    echo "Creates a customized fork of the auto-tune workspace at the given path."
    exit 1
fi
...
echo ""
echo "$(color '1;36' '╔══════════════════════════════════════════════╗')"
echo "$(color '1;36' '║')  Auto-Tune Workspace Fork Setup              $(color '1;36' '║')"
echo "$(color '1;36' '╚══════════════════════════════════════════════╝')"
echo ""
...
ask "Project name (used in CLAUDE.md and docker-compose)" "my-project"
PROJECT_NAME="$REPLY"

ask "Short description of the project's purpose" "A self-improving Claude Code workspace"
PROJECT_DESC="$REPLY"
...
# Update service name from claude_auto_tune to project name
SANITIZED_NAME="$(echo "$PROJECT_NAME" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9_-]/_/g')"
sed -i "s/claude_auto_tune-claude/${SANITIZED_NAME}-claude/g" "$TARGET_DIR/run.sh" 2>/dev/null || true
...
git add -A
git commit -q -m "Initial workspace setup from auto-tune template
...
echo ""
echo "  Don't forget to add your secrets in GitHub repo settings:"
echo "    - CLAUDE_CODE_OAUTH_TOKEN"
echo "    - ANTHROPIC_API_KEY (if using direct API)"
echo ""
