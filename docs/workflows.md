...
`auto-improve-verify.yml` runs on the cron schedule defined in `auto_improve_verify.schedule` (daily by default, and dispatchable with an `issue_number` input to verify one issue at a time). For each target issue it re-runs the extractor subagent scoped to that issue's fingerprint key, produces a fresh "after" snapshot, compares it to the frozen `## Baseline (before fix)` section, and appends a `## Verification history` entry. It is the only workflow allowed to advance issues to `auto-improve:merged`, close them as `auto-improve:solved`, or reopen them on regression. Closure requires `tracking.verify_runs` successive clean verify runs (default: 1 — one targeted clean run is enough, since the comparison is per-issue rather than global).
