...
- `hub/fetch-local-transcripts.py` — **deterministic** fetcher (the pull half). Run by CI workflows before the Claude step to populate `.scratch/hub-transcripts/` with session JSONL files from the hub. The `workflow-insights-extractor` subagent parses this directory alongside CI artifacts. Hard no-op when hub config is off or `HUB_READ_TOKEN` is missing. Auth is scoped: the `HUB_READ_TOKEN` env var is only visible to this step. No LLM calls.
