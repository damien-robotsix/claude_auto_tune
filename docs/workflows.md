...
- `hub/push-ci-transcripts.py` — **deterministic** publisher for CI Claude Code session transcripts. Runs as a post-session step in every CI workflow and pushes session JSONL files to the hub repo under `transcripts/<workspace-slug>/<YYYY-MM-DD>/` with `source: "ci"` metadata (including workflow name, run ID, and actor). Uses `HUB_TOKEN` for auth (scoped to this step only). Best-effort: always exits 0 so it never fails a workflow. Gated on `hub.ci_transcripts.enabled`. No LLM calls.
...
- `hub-daily-sweep-prompt.md` — the prompt used by the hub-daily-sweep workflow to decide which recent commits are generalizable.
- `hub/hub-open-proposal.py` — **deterministic** helper that opens a proposal issue in the hub repo for a given commit. No LLM calls.
- `hub/hub-search.py` — **deterministic** helper that searches existing hub proposals to avoid duplicates. No LLM calls.
- `hub/list-recent-commits.py` — **deterministic** helper that lists commits on the default branch within a lookback window. No LLM calls.
