...
2. **Verify** (`auto-improve-verify.yml`, driven by `scripts/auto-improve-verify-prompt.md`) — runs per-issue, either on its own daily cron or on demand with an `issue_number` input. For each target issue it re-invokes the extractor subagent scoped to that issue's fingerprint key, produces a fresh "after" snapshot, appends a `## Verification history` entry comparing it to the frozen baseline, and — uniquely — owns the transitions to `auto-improve:merged`, `auto-improve:solved` (closed), and regression reopens. Closure requires `tracking.verify_runs` successive clean verify runs (default: 1).
