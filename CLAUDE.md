...
- **Do NOT use the `Agent` tool during code reviews.** Every task in a review (reading files, searching for definitions, checking diffs, looking up context) can be handled with direct tool calls (`Read`, `Grep`, `Glob`, `Bash`). Agent subagents add ~2-5k tokens of overhead each and provide no benefit for review tasks. If you find yourself about to spawn an Agent, stop and use the direct tool instead.
